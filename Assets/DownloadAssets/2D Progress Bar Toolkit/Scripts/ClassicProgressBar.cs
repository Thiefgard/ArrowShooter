...
public class ClassicProgressBar : MonoBehaviour {
	...
	public void Awake() {
		...
		for (int i = 0; i < m_NumberOfSegments; i++) {
			GameObject currentSegment = Instantiate(m_Image.gameObject, transform.position, transform.rotation, transform);
			currentSegment.transform.rotation = Quaternion.Euler(0, 0, 90f);
			...
			segmentRectTransform.position += (Vector3.up * i * (m_SizeOfSegment -150f) ) - (Vector3.up * m_SizeOfSegment * (m_NumberOfSegments / 2))/* + (Vector3.up * i * m_SizeOfNotch)*/;
